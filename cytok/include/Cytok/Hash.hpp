...
namespace ck
{
    namespace utils
    {
        ...
        struct hash_impl
        {
            typedef typename uint_t<n>::type ReturnType;
        ...
        struct hash
        {
            ...
            {
                return hash_impl<n>::get((const unsigned char*)value.c_str() , value.length());
            }
            
            static ReturnType get(const char* value)
            {
                return hash_impl<n>::get((const unsigned char*)value , strlen(value));
            }
            
            template<typename T>
            static ReturnType get(const T value,int8_t offset = 0)
            {
                size_t size = sizeof(T);
                
                const uint8_t* val = reinterpret_cast<const uint8_t*>(&value);
                
                return hash_impl<n>::get(val,size);
            }
        };
